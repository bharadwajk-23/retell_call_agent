...
def append_call_log(log_entry: Dict[str, Any]) -> None:
    ...
    save_json_file(config.CALL_LOGS_PATH, logs)


def find_patient_by_phone(phone: str) -> Optional[Dict[str, Any]]:
    norm = normalize_phone(phone)
    patients: List[Dict[str, Any]] = load_json_file(config.PATIENT_DETAILS_PATH)
    for p in patients:
...
def _book_appointment_record(
    ...
    save_json_file(config.APPOINTMENTS_PATH, appointments)
